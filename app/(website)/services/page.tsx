...
import { getAllPublicServices } from '@/lib/actions/public';
...
export default function ServicesPage() {
  ...
  const { query, setQuery, filtered } = useSearchFilter(services, (s) => [s.name, s.description, s.price, `${s.duration} min`]);
  const { pageItems, page, setPage, totalPages } = usePagedList(filtered, 9);

  useEffect(() => {
    // Public action: only Active services, only public fields
      ...
      <div className="max-w-7xl mx-auto px-6 w-full">
        ...
        {!loading && services.length > 0 && (
          <div className="max-w-xl mx-auto mb-8 md:mb-12">
            <SearchBar
              ...
              resultText={`${filtered.length} of ${services.length} services`}
            />
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="text-center text-zinc-500 py-16 w-full">
            <p className="mb-4">No services match &ldquo;{query}&rdquo;.</p>
            <button type="button" onClick={() => setQuery('')} className="text-xs font-bold uppercase tracking-widest text-amber-600 dark:text-amber-500 hover:underline">Clear search</button>
          </div>
        ) : (
            ...
            {pageItems.map((service, index) => (
              <ScrollReveal key={service.id || index} direction="down" delay={(index % 3) * 0.15} className="flex flex-col h-full">
                <div className="group bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 hover:border-amber-500/50 transition-colors duration-500 shadow-sm hover:shadow-md dark:shadow-none flex flex-col h-full w-full">
                  <div className="h-48 sm:h-64 overflow-hidden relative w-full flex items-center justify-center bg-zinc-100 dark:bg-zinc-950">
